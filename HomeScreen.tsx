...
// ─── SVG LOGO COMPONENT (extracted from your SVG file) ───
const TransEatLogo = ({ width = 120, height = 120 }: { width?: number; height?: number }) => (
  <Svg width={width} height={height} viewBox="150 550 1000 350">
    {/* Trans letters — light blue (#DDE9F1) */}
...
const HomeScreen = ({ onNavigate, onSelectRestaurant }: { onNavigate: (screen: string) => void; onSelectRestaurant?: (id: string, name: string) => void }) => {
  ...
  const runBurgerCycle = useCallback(() => {
    // Phase 1: Burger visible next to logo for 60 seconds
    burgerTranslateX.setValue(0);
    burgerOpacity.setValue(1);

    setTimeout(() => {
      // Phase 2: Burger cruises off to the right
      Animated.parallel([
        ...
        // Phase 3: Burger hidden for 30 seconds, then pops up next to word
        setTimeout(() => {
          // Reset to left side (next to logo)
        ...
        }, 30000); // 30 seconds hidden
      });
    }, 60000); // 60 seconds visible
  }, [width]);
  ...
  const handleSearchSubmit = () => {
    if (searchText.trim()) {
      localStorage.setItem('searchTerm', searchText.trim());
  ...
  const handleCategoryPress = (categoryId: string) => {
    ...
    localStorage.setItem('filterCategory', categoryId);
    localStorage.removeItem('searchTerm');
    goOrderFood();
  };

  const handleFoodTypePress = (foodType: string) => {
    localStorage.setItem('searchTerm', foodType);
    localStorage.removeItem('filterCategory');
    goOrderFood();
  };
  ...
  const handleItemPress = (item: MenuItem) => {
    localStorage.setItem('highlightItem', item.name);
  ...
  return (
    <View style={styles.container}>
      {/* Header background — TINY strip, ALMOST INVISIBLE when scrolled */}
      <View style={[styles.headerBackground, isScrolled && styles.headerBackgroundGlass]} />
